...
- **SUMUI-014A**: SUMUI-014 is resolved by the following canonical mapping. The valid label vocabulary is 「主要」「有力」「補助」「微小」. Earlier draft vocabulary 「必須」「重要」 is superseded and MUST NOT be emitted by new v8.4 artifacts.
- **SUMUI-014B**: For an adopted inference element, importance is assigned deterministically from its final dependency-safe quantitative role:
  - 「主要」: non-suppressed adopted element with `SelectionScore >= 20`;
  - 「有力」: non-suppressed adopted element with `SelectionScore >= 10` and <20;
...
- **SUMUI-014C**: An ALTERNATIVE/FALLBACK element suppressed by `DO_NOT_MULTIPLY` does not inherit a stronger label merely from its standalone score. If retained as an active fallback inference path, its importance is one tier below the score-derived tier, with a floor of 「微小」. If it is not an active inference path, it receives no adopted importance label.
- **SUMUI-014D**: Evidence constraints are not assigned these numeric importance labels unless a future Manifest rule defines an Evidence-specific quantitative usefulness measure.
- **SUMUI-014E**: An adopted LIVE_CONDITIONAL element with a permitted single-value benchmark exposure follows the ordinary score-derived rule. If exposure is unresolved but a separate v8.4 runtime-policy route admits the element, its provisional importance is 「補助」.
- **SUMUI-014F**: For an exact-runtime-exposure candidate whose 7000G benchmark is only an upper bound, the upper bound MUST NOT be treated as SelectionScore and MUST NOT upgrade importance. Preserve `MAXIMUM_SELECTION_SCORE` as diagnostic evaluation with status `UPPER_BOUND_ONLY`; user-facing label is 「設定判別スコア上限」.
- **SUMUI-014G**: When the permitted evaluation route is per eligible trial, compute `PER_ELIGIBLE_TRIAL_POWER = IGPerEligibleTrial × 200` and expose it as 「1回の判別力」. v8.4 defines no adoption threshold for this metric; until a later policy defines one, admission through this route carries provisional importance 「補助」.
- **SUMUI-014H**: The ordinary numeric adoption threshold remains `SelectionScore >= 5.0`. Importance labels do not change that threshold.

### 25.6 Canonical UI propagation of evaluation semantics
- **SUMUI-016**: Machine Research Summary MUST preserve, for every adopted numeric element, `featureId`, canonical importance, quantitative evaluation metric/value/status when available, and the user-facing metric label. Downstream stages MUST preserve these fields without reclassification.
- **SUMUI-017**: Canonical UI MUST bind each numeric input section to the adopted feature(s) it presents. A section containing at least one active numeric inference element MUST carry a section-header importance derived upstream; the Renderer MUST display it immediately before the Accordion chevron and MUST NOT infer it from input roles.
- **SUMUI-018**: If one section contains multiple active numeric features, the section-header badge is the strongest canonical importance among those features in the fixed order 主要 > 有力 > 補助 > 微小. Individual feature evaluations remain independently visible in Machine Research Summary; the header badge is presentation aggregation only and MUST NOT alter inference semantics.
- **SUMUI-019**: Evidence-only sections and sections without an active numeric inference element MUST NOT receive a numeric importance badge.
- **SUMUI-020**: Machine Research Summary evaluation rows are independent structured UI data. `MAXIMUM_SELECTION_SCORE` MUST render as an independent 「設定判別スコア上限：x.x」 row; including the same value only inside reason prose is insufficient. `PER_ELIGIBLE_TRIAL_POWER` analogously renders as 「1回の判別力：x.x」 when present.
- **ASSERT-UI-008**: Runtime Contract Verification MUST fail closed if an adopted numeric section loses its canonical importance, if a summary evaluation metric/value is dropped, or if a Renderer would need to reconstruct either from machine-specific knowledge.
