...
SEL-008D: The user-facing name for `IGPerEligibleTrial × 200` is 「1回の判別力」. It is a per-observation information-strength indicator, not a 7000G practical-usefulness score. It MUST be shown only when the underlying setting-specific outcome model is complete enough to compute `IGPerEligibleTrial` deterministically. It MUST NOT be displayed as 「設定判別スコア」 or compared numerically as though the two metrics had the same denominator.

SEL-008E: Manifest v8.4 defines no adoption/rejection threshold for 「1回の判別力」. The metric is CALIBRATION-stage quantitative metadata. A LIVE_CONDITIONAL candidate with non-zero per-trial information is not rejected merely because its current 「1回の判別力」 is numerically small. Future Manifest versions MAY define thresholds only after cross-machine calibration; such a policy change MUST be versioned and mechanically re-evaluable from preserved raw metrics.

SEL-008F: If the setting-specific outcome distribution is incomplete or otherwise insufficient to compute `IGPerEligibleTrial`, the user-facing summary MUST show that the per-observation metric is 「算出不可」 and explain the concrete reason in natural language. Missing information MUST NOT be replaced by zero or by an assumed distribution.

SEL-008G: Evaluation, Research Eligibility, Runtime Activation and Presentation are separate contracts. For every evaluated Numeric candidate, preserve (a) the raw evaluation metric/value and calculation version, (b) Research Eligibility stating whether the feature is statistically and observationally valid for inference independent of product thresholds, and (c) runtime activation derived mechanically from Eligibility plus the distributed Runtime Policy. Changing a Runtime Policy threshold MUST NOT alter Research facts, overwrite the raw evaluation value, or require Selection/MachineData regeneration merely to toggle an already-eligible feature.

SEL-008H: Thresholds that control whether an already-eligible evaluated feature is currently used by the product MUST be represented in one versioned, machine-independent Runtime Policy. The policy MUST support at least the benchmark 「設定判別スコア」 metric and the per-observation 「1回の判別力」 metric as distinct threshold domains. Manifest v8.4 defines no minimum Runtime Policy threshold for 「1回の判別力」 during Calibration; a later threshold may be introduced after cross-machine evidence accumulates.

SEL-008I: Runtime Policy is an activation filter, not a research or statistical-validity authority. An INELIGIBLE feature caused by incomplete likelihood, invalid denominator, dependency/double counting, unresolved observation semantics, or another Research/Eligibility defect MUST NOT become active merely because its numeric value passes a Runtime Policy threshold. Only ELIGIBLE features may transition between ACTIVE and INACTIVE_BY_POLICY.

SEL-008J: Runtime Policy thresholds MUST be changeable independently of Research, Evaluation, Selection construction, MachineData regeneration and APK release when the runtime contract is unchanged. The App/Renderer MUST NOT invent, tune or reinterpret those thresholds; it may only consume the distributed policy and mechanically apply the defined comparison to preserved machine-readable evaluation values. Returning the policy to an earlier threshold MUST restore an otherwise eligible feature without new Research.

SEL-008K: A feature that becomes INACTIVE_BY_POLICY remains present in Research/Evaluation/MachineData with its raw metric and Eligibility unchanged. It MUST be excluded from ProbabilityEngine inference and hidden from the ordinary inference-input UI. In 「この機種の設定推測について」 it MUST move to the user-facing rejected/not-currently-used group with its preserved metric and a natural-language explanation that it is below the current adoption standard. If the policy makes it ACTIVE again, both inference participation and the adopted-summary placement MUST be restored mechanically.

SEL-008L: Runtime activation MUST propagate through feature-to-input bindings rather than relying on feature IDs being present on Canonical UI nodes. When an inactive feature owns numerator, denominator or categorical outcome inputs, those inputs MUST be hidden unless at least one ACTIVE feature still requires the same input. Shared denominators or other shared inputs therefore remain visible while any ACTIVE dependent feature needs them. For categorical interactions, only inactive bound categories are removed; if no categories remain, the interaction is removed. Structural UI groups that become empty solely because all policy-bound descendants were removed MUST also be omitted. This propagation is generic and machine-independent; machine-specific Runtime Policy UI branches are prohibited.

SEL-008M: MachineData produced before the v8.4 Eligibility/Evaluation runtime contract MAY omit Eligibility metadata. Runtime Policy MUST NOT reinterpret such legacy absence as INELIGIBLE or apply v8.4 threshold deactivation to it. This compatibility rule exists only to leave legacy packages unaffected; all newly produced v8.4 Numeric inference features MUST carry explicit Eligibility and Evaluation according to SEL-008G.
...
SEL-013: User-facing Selection explanations MUST use 「設定判別スコア」 for the 7000G benchmark SelectionScore and 「1回の判別力」 for the per-observed-trial metric defined by SEL-008D. If neither metric is computable, the explanation MUST state 「算出不可」 with the reason. Every adopted Numeric element therefore exposes either a quantitative evaluation or a concrete reason why quantitative evaluation is unavailable.
SEL-014: Internal Research/Selection vocabulary such as CORE, SUPPORT, JOINT_ELIGIBLE, LIVE_CONDITIONAL, Exposure class names, schema IDs, and unresolved implementation prose MUST NOT be emitted verbatim in end-user explanatory copy. Canonical UI/Machine Research Summary MUST provide natural Japanese presentation text while retaining the exact internal status separately as machine-readable metadata.
...
- **SUMUI-012**: Every adopted setting-inference element MUST additionally carry a user-facing qualitative importance label in natural Japanese. The canonical label vocabulary is fixed as: 「主要」「有力」「補助」「微小」. Runtime renderers MUST display the canonical label and MUST NOT derive or rename it independently.
...
- **SUMUI-014**: Importance mapping is deterministic and machine-independent. For an ACTIVE adopted Numeric element evaluated by 「設定判別スコア」, assign 「主要」 when the unrounded score is >= 20.0, 「有力」 when >= 10.0 and < 20.0, 「補助」 when >= 5.0 and < 10.0, and 「微小」 when < 5.0. For an ACTIVE adopted Numeric element whose valid quantitative evaluation is 「1回の判別力」 under SEL-008D, Manifest v8.4 assigns 「補助」 provisionally regardless of the numeric value because no calibrated importance boundary exists yet for that metric. This provisional mapping MUST NOT be interpreted as an adoption threshold. Machine-specific freehand importance labeling is prohibited.
...
- **SUMUI-016**: Inside 「この機種の設定推測について」, the blocks 「採用した設定推測要素」「採用しなかった要素」「未解決の要素」 MUST each be collapsible when present. Individual setting-inference elements inside those blocks MUST also use a compact disclosure row when detailed reason/metric text is available, preserving a simple collapsed view.
- **SUMUI-017**: For an adopted element's collapsed disclosure row, the canonical importance label 「主要」「有力」「補助」「微小」 MUST be shown at the right edge immediately before the disclosure affordance. Importance is not an input-required marker. Rejected, unresolved, or INACTIVE_BY_POLICY elements MUST NOT retain an adopted-importance badge in their collapsed row.
- **SUMUI-018**: Expanding an evaluated Numeric element MUST show exactly the applicable quantitative user-facing metric: 「設定判別スコア」 when benchmark exposure supports SelectionScore, or 「1回の判別力」 when the element is evaluated per eligible observation under SEL-008D. The two metrics MUST NOT be presented as interchangeable rankings. User-facing display rounds to one decimal place; internal evaluation and Runtime Policy comparison MUST use the unrounded stored value.
- **SUMUI-019**: When neither quantitative metric is valid, the expanded explanation MUST show 「算出不可」 plus the concrete natural-language reason. It MUST NOT expose internal Research/Selection tokens.
- **SUMUI-020**: Evidence/Numeric computational differences SHOULD be explained in the existing natural-language explanation where relevant; the ordinary input UI MUST NOT add badges, extra classification cards, or other persistent decoration solely to expose this distinction. Simplicity of the primary input surface is authoritative.
- **SUMUI-021**: A Runtime Policy activation change MUST update the summary classification generically for every affected 「1回の判別力」 feature, not through machine-specific presentation patches. ACTIVE eligible elements appear under 「採用した設定推測要素」; INACTIVE_BY_POLICY elements appear under 「採用しなかった要素」 with their preserved 「1回の判別力」 and current-policy reason.
...
### INPUTUX-007 — 親子階層の同名見出しを生成しない
セクション、グループ、入力カード、入力コントロールが親子関係にあり、子の見出しが親と同一または実質的に同義で追加の観測文脈を与えない場合、その中間見出し MUST NOT be rendered. 例えば「通常時」セクション直下に文脈追加のない「通常時」グループを置いたり、「通常ゲーム数」カード内で入力コントロールに再び「通常ゲーム数」と表示したりしてはならない。

### INPUTUX-008 — 主ラベルは最も外側の十分な位置に1回だけ表示する
各入力対象の主ラベルは、その入力を一意に識別できる最も外側の表示位置に原則1回だけ置く。内側のRenderer componentがアクセシビリティ名や内部識別子を必要とする場合も、可視ラベルを重複させてはならない。必要な accessible name は非重複のアクセシビリティ属性として保持する。

### INPUTUX-009 — グループは意味を追加するときだけ可視化する
可視グループ見出しは、親セクションだけでは表せない自然な観測文脈、対象範囲、条件、または操作上のまとまりを追加するときに限り生成する。単一の子要素を親と同名で包むためだけのグループ、または schema/component 階層をそのまま露出するだけのグループは生成してはならない。

### ASSERT-UI-007 — 実機入力操作性の fail-closed 検証
Preview / Runtime Contract Verification は、(a) 大きな母数が直接入力可能、(b) UNOBSERVED と OBSERVED_ZERO が区別される、(c) 未観測表示が重複しない、(d) 同一入力ラベルが不必要に反復されない、(e) 親子階層に同名・同義の可視見出しが重複しない、(f) 可視グループが実際の観測文脈を追加していることを確認する。違反時は機種固有UIで補正せず、Canonical規則または汎用Rendererを修正する。
...
- **SUMUI-014A**: SUMUI-014 is resolved by the following canonical mapping. The valid label vocabulary remains SUMUI-012: 「主要」「有力」「補助」「微小」. These are absolute usefulness labels, not input requirements or relative rankings; 「主要」 does not imply that multiple competing elements must exist.
- **SUMUI-014B**: For an adopted inference element, importance is assigned deterministically from its final dependency-safe quantitative role:
  - 「主要」: preferred primary with `SelectionScore >= 20`;
  - 「有力」: non-suppressed adopted element with `SelectionScore >= 10`, or a preferred primary whose score is 10–<20;
