...
SEL-008G: Evaluation value, Selection disposition and Presentation visibility are separate contracts. For every evaluated Numeric candidate, preserve (a) the raw evaluation metric/value and calculation version, (b) the current Selection disposition and reason under a versioned Selection Policy, and (c) the downstream presentation visibility derived from the current disposition/policy. Changing a threshold MUST NOT delete Research or overwrite the historical raw evaluation value.

SEL-008H: Selection thresholds MUST be represented as versioned policy data rather than embedded only as irreversible machine-level decisions. For the v8.4 benchmark metric, the governing policy remains SelectionScore = IG7000 × 200 with CORE >= 20, SUPPORT >= 10, JOINT_ELIGIBLE >= 5, below 5 = REJECT. A future threshold revision MUST permit deterministic fleet-wide re-evaluation from preserved scores without repeating Research when the underlying Research facts and metric definition are unchanged.

SEL-008I: App/Renderer MUST NOT independently choose or modify Selection thresholds. Threshold changes belong to the governed Selection Policy and are applied upstream to regenerate Selection/MachineData. Distribution may then make resulting inclusions/exclusions or visibility changes available to installed apps without requiring a new APK when the runtime contract itself is unchanged.

SEL-008J: A candidate excluded by a future threshold policy MUST remain in Research/Evaluation history with its raw metric, policy version, disposition and reason. Presentation MAY hide it from inference input after upstream re-evaluation, and a later policy revision MAY restore it without new Research if the preserved evidence remains valid.
