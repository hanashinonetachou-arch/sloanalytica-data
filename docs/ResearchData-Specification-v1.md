...
## MultinomialのdistributionMode
- `complete`: 明示カテゴリが全結果を覆い、確率合計は1。
- `implicit_residual`: MachineDataの実運用に合わせ、明示カテゴリ以外を残余カテゴリとして扱う。各設定の残余確率は `1 - 明示カテゴリ確率合計` としてEvaluator内部でのみ生成する。
- 明示カテゴリ自体が欠損している場合は補完しない。
