...
const eventInputs=f=>uniq([
  f?.numeratorInputId,
  ...arr(f?.numeratorInputIds),
...
const collectResearchTargets=(node,out=[])=>{
  if(Array.isArray(node)){
    for(const v of node) collectResearchTargets(v,out);
    return out;
  }
...
const rawTokenLeak=(name,categories)=>{
  ...
  for(const c of categories){
    if(typeof c==='string' && /^[A-Z][A-Z0-9_]{2,}$/.test(c) && name.includes(c)) return true;
  }
  return /(?:^|[\s（(])(?:[A-Z][A-Z0-9_]{2,})(?:$|[\s）)])/u.test(name);
};

const report={
  schemaVersion:'shared-feature-evidence-horizontal-audit-v1',
  generatedAt:new Date().toISOString(),
  targets:[],summary:{PASS:0,REVIEW:0,HIGH_RISK:0},issues:[]
};
...
for(const machineId of TARGETS){
  ...
  const selInputById=new Map(arr(selection.inputs).map(i=>[i.id,i]));
  const pkgFeatures=arr(pkg?.features?.features);
  const pkgFeatureById=new Map(pkgFeatures.map(f=>[f.featureId,f]));
  const pkgEvidence=arr(pkg?.evidence?.evidences);
  const machineIssues=[];

  // A. Candidate discovery from Research researchTarget links.
  const targetSets=collectResearchTargets(research.discoveryInventory??research);
  const pairs=new Map();
  for(const ids of targetSets){
    const rfs=ids.filter(id=>rfById.has(id));
    const res=ids.filter(id=>reById.has(id));
  ...
  for(const {researchFeatureId,researchEvidenceId} of pairs.values()){
    const rf=rfById.get(researchFeatureId);
    const sf=sfByResearchId.get(researchFeatureId);
    const se=seByResearchId.get(researchEvidenceId);
    if(!sf||!se) continue;
    if(!hasNumericResearch(rf)) continue;
    ...
    if(!events.includes(se.inputId)){
      machineIssues.push({severity:'REVIEW',code:'SHARED_SURFACE_SEPARATE_INPUTS',researchFeatureId,researchEvidenceId,featureId:sf.featureId,evidenceId:se.evidenceId,featureEventInputs:events,evidenceInputId:se.inputId});
    }
  }

  // B. Explicit sharedFeatureIds contract integrity in Selection and MachinePackage.
  for(const e of arr(selection.evidence)){
    ...
    for(const featureId of declared){
      ...
      if(!sf||!INCLUDE.has(sf.adoptionCategory)||!eventInputs(sf).includes(e.inputId)){
        machineIssues.push({severity:'HIGH_RISK',code:'INVALID_SELECTION_SHARED_CONTRACT',evidenceId:e.evidenceId,inputId:e.inputId,sharedFeatureId:featureId});
      }
    }
    const pe=pkgEvidence.find(x=>x.id===e.evidenceId);
    if(pkg && (!pe || JSON.stringify(uniq(arr(pe.sharedFeatureIds)).sort())!==JSON.stringify(declared.slice().sort()))){
      machineIssues.push({severity:'HIGH_RISK',code:'SHARED_CONTRACT_NOT_PROPAGATED',evidenceId:e.evidenceId,selectionSharedFeatureIds:declared,packageSharedFeatureIds:uniq(arr(pe?.sharedFeatureIds))});
    }
  }

  // C. MachinePackage overlap that is not explicitly contracted.
  const featureUsers=new Map();
  for(const f of pkgFeatures){
    for(const inputId of eventInputs(f)){
  ...
  for(const e of pkgEvidence){
    ...
    if(!overlaps.every(id=>declared.includes(id))){
      machineIssues.push({severity:'HIGH_RISK',code:'UNDECLARED_PACKAGE_FEATURE_EVIDENCE_OVERLAP',evidenceId:e.id,inputId:e.inputId,featureIds:overlaps,sharedFeatureIds:declared});
    }
  }

  // D. User-facing raw/internal category token leakage.
  const researchCategories=uniq(arr(research.features).flatMap(f=>arr(f.categories)));
  for(const input of arr(selection.inputs)){
    if(rawTokenLeak(input.name,researchCategories)) machineIssues.push({severity:'REVIEW',code:'USER_FACING_INTERNAL_TOKEN',inputId:input.id,name:input.name});
  }
  // Also scan UI JSON strings that are most likely labels/titles without treating IDs as labels.
  const uiStrings=[];
  const walkUi=(node,key='')=>{
    if(Array.isArray(node)) return node.forEach(v=>walkUi(v,key));
  ...
  for(const s of uiStrings){
    if(rawTokenLeak(s.value,researchCategories)) machineIssues.push({severity:'REVIEW',code:'UI_INTERNAL_TOKEN',key:s.key,value:s.value});
  }
...
for(const t of report.targets){
  if(t.severity!=='PASS') console.log(`${t.severity}: ${t.machineId} (${t.issueCount})`);
}
const counts={};
for(const i of report.issues) counts[i.code]=(counts[i.code]??0)+1;
