...
function projectRuntimeCandidates(candidates,runtimePolicy){
  ...
  return (candidates??[]).map(candidate=>{
    ...
    const metric=binding.metric??candidate.evaluation?.metric;
    if(metric!==candidate.evaluation?.metric) return {...clone(candidate),runtimeStatus:'INACTIVE',runtimeReason:'RUNTIME_POLICY_BINDING_MISMATCH'};
    ...
    return {...clone(candidate),runtimeStatus:value>=threshold?'ACTIVE':'INACTIVE',runtimeReason:value>=threshold?'THRESHOLD_MET':'THRESHOLD_NOT_MET'};
  });
}
...
export function compileV8MachinePackage({research,selection,observation,evidence,highLow,summary,canonical,runtimePolicy,materializeUi}){
  ...
  const runtimeProjection=projectRuntimeCandidates(candidates,runtimePolicy);
