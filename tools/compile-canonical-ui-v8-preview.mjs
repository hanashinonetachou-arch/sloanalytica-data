...
export function validateCanonicalUiV8(ui){
  ...
  for(const s of ui?.sections??[]){
    ...
    for(const n of nodes){
      ...
      if(n.observationMultiplicity==="REPEATED_CATEGORICAL_EVENT" && (n.interaction?.type!=="ADD_OBSERVATION"||n.interaction?.preservePriorObservations!==true)) errors.push(`${s.id}/${n.id??n.evidenceId}: repeated categorical event must accumulate`);
    }
