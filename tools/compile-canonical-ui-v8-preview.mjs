...
export function validateCanonicalUiV8(ui){
  ...
  for(const s of ui?.sections??[]){
    ...
    const checkRepeated=(n)=>{if(n.observationMultiplicity==="REPEATED_CATEGORICAL_EVENT"){const t=n.interaction?.type;if(!["CATEGORY_COUNTERS","ADD_OBSERVATION"].includes(t)||n.interaction?.preservePriorObservations!==true||n.interaction?.showAccumulatedCounts!==true) errors.push(`${s.id}/${n.id??n.evidenceId}: repeated categorical event needs an explicit accumulating interaction`);if(t==="CATEGORY_COUNTERS"){const coverage=n.interaction?.categoryCoverage;if(!["EXHAUSTIVE","NON_EXHAUSTIVE"].includes(coverage)) errors.push(`${s.id}/${n.id??n.evidenceId}: categoryCoverage required`);if(coverage==="EXHAUSTIVE"&&n.interaction?.totalOpportunities!=="DERIVE_FROM_CATEGORY_COUNTS") errors.push(`${s.id}/${n.id??n.evidenceId}: exhaustive category counters must derive total opportunities`);if(coverage==="NON_EXHAUSTIVE"){const ot=n.interaction?.opportunityTracking;if(n.interaction?.totalOpportunities!=="SEPARATE_COUNTER"||ot?.type!=="SEPARATE_COUNTER"||!ot?.inputId||!ot?.label) errors.push(`${s.id}/${n.id??n.evidenceId}: non-exhaustive category counters require separate opportunity tracking`);}const cats=n.interaction?.categories;if(!Array.isArray(cats)||cats.length===0) errors.push(`${s.id}/${n.id??n.evidenceId}: category counters require canonical categories`);else {const catIds=new Set();for(const cat of cats){if(!cat?.id||!cat?.label||!cat?.meaning||catIds.has(cat.id)) errors.push(`${s.id}/${n.id??n.evidenceId}: category id/label/meaning missing or duplicate`);catIds.add(cat.id);}}}}};
