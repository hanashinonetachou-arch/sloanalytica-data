...
if(process.argv[1]&&path.resolve(process.argv[1])===path.resolve(import.meta.filename)){for(const arg of process.argv.slice(2)){const [target,features]=arg.split(":");const isRepro=target.startsWith("repro-v8/");const id=isRepro?target.slice("repro-v8/".length).replace(/\/$/,""):target;console.log(JSON.stringify({machineId:id,features:features?.split(",")??"PRIMARY",source:isRepro?"REPRO_V8":"RESEARCH",results:calculate(id,20000,20260920,features?.split(","),isRepro?target:null)},null,2));}}
