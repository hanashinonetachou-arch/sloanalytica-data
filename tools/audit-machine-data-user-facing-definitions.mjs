...
for (const d of fs.readdirSync(machinesDir,{withFileTypes:true}).filter(x=>x.isDirectory()).sort((a,b)=>a.name.localeCompare(b.name))) {
  ...
  for(const x of arr(s.inputs).filter(x=>firstHit(x.name))){
    const linked=arr(s.features).filter(f=>selected(f)&&f.numeratorInputId===x.id);
    ...
    for(const f of linked){
      ...
      if(!denomId){
        flags.push({severity:'HIGH',code:'FIRST_HIT_DENOMINATOR_MISSING',inputId:x.id,featureId:f.featureId,detail:`「${x.name}」の採用Featureに分母入力がありません`});
      } else if(!denom || (!explicitGameBasis(denomName) && !/(分母|対象|除外|通常|ゲーム|G数)/.test(context))){
        flags.push({severity:'REVIEW',code:'FIRST_HIT_DENOMINATOR_CONTEXT_WEAK',inputId:x.id,featureId:f.featureId,denominatorInputId:denomId,detail:`「${x.name}」の分母「${denomName || denomId}」の対象区間がユーザー向け文言から判別しにくいです`});
      }
      if(!clean(x.description)) info.push({code:'FIRST_HIT_DESCRIPTION_ABSENT',inputId:x.id,featureId:f.featureId});
    }
  }
...
const report={schemaVersion:'machine-data-user-facing-definitions-audit-v2',generatedAt:new Date().toISOString(),summary,machines:rows.sort((a,b)=>{const w=x=>x.status==='HIGH_RISK'?2:x.status==='REVIEW'?1:0;return w(b)-w(a)||b.flags.length-a.flags.length||a.machineId.localeCompare(b.machineId);})};
const outJson=path.resolve(process.argv[3] ?? path.join(root,'reports','machine-data-user-facing-definitions-audit-v2.json'));
const outMd=path.resolve(process.argv[4] ?? path.join(root,'reports','machine-data-user-facing-definitions-audit-v2.md'));
fs.mkdirSync(path.dirname(outJson),{recursive:true});
fs.writeFileSync(outJson,JSON.stringify(report,null,2)+'\n');
const md=['# MachineData User-facing Definitions Audit — v2','',`- Machines: ${summary.machineCount}`,`- PASS: ${summary.pass}`,`- REVIEW: ${summary.review}`,`- HIGH_RISK: ${summary.highRisk}`,'','## Flag counts','',...Object.entries(summary.flagCounts).map(([k,v])=>`- ${k}: ${v}`),'','## Priority','','| # | Machine | Status | Flags |','|---:|---|---|---|',...report.machines.filter(r=>r.status!=='PASS').slice(0,80).map((r,i)=>`| ${i+1} | ${r.displayName} (${r.machineId}) | ${r.status} | ${r.flags.map(f=>f.code).join(', ')} |`),'','## Interpretation','','- HIGH_RISK/REVIEWは文言・定義の再確認優先度であり、自動的に計算ロジックが誤りという意味ではありません。','- 初当り入力にdescriptionが無いだけでは警告せず、分母入力そのものが不明瞭な場合だけ優先候補にします。','- 短い理由を一律に悪いとせず、内容が抽象語だけの場合に限定して警告します。','- この監査はMachineDataを変更しません。'].join('\n')+'\n';
fs.writeFileSync(outMd,md);
console.log(`User-facing definitions audit v2: PASS ${summary.pass} / REVIEW ${summary.review} / HIGH_RISK ${summary.highRisk} / TOTAL ${summary.machineCount}`);
