...
function buildFeature(rf,sf,inputIds,inferenceSettings=null){
  ...
  if(rf.candidateModel==="binomial" || rf.candidateModel==="poisson"){
    ...
    base.probabilities=Object.fromEntries(Object.entries(rf.settingValues??{})
      .filter(([setting])=>!Array.isArray(inferenceSettings)||inferenceSettings.includes(setting))
      .map(([setting,v])=>[setting,v.probability]).filter(([,p])=>Number.isFinite(p)));
    ...
    base.categoryProbabilities=Object.fromEntries(Object.entries(rf.settingDistributions??{})
      .filter(([setting])=>!Array.isArray(inferenceSettings)||inferenceSettings.includes(setting))
      .map(([s,dist])=>{
...
export function buildMachineData(research,selection,statistics=null){
  ...
  const inferenceSettings=Array.isArray(research.machine?.inferenceSettings)?research.machine.inferenceSettings:research.machine?.settings??[];
  const features=[];
  for(const sf of selection.features??[]){
    const rf=rfs.get(sf.researchFeatureId);
    if(!rf) fail(`unknown researchFeatureId: ${sf.researchFeatureId}`);
    const built=buildFeature(rf,sf,inputIds,inferenceSettings); if(built) features.push(built);
  }
