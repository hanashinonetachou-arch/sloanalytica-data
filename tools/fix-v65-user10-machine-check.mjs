...
// 3) Batch quality: suppress only warnings/conflicts that downstream policy has explicitly resolved.
{
  ...
  const oldConflicts='  if ((research?.conflicts ?? []).length) reasons.push(`Research conflicts: ${research.conflicts.length}`);';
  const newConflicts='  const unresolvedConflicts = (research?.conflicts ?? []).filter(shouldSurfaceResearchConflict);\n  if (unresolvedConflicts.length) reasons.push(`Research conflicts: ${unresolvedConflicts.length}`);';
  if(!text.includes(oldConflicts)) throw new Error('batch conflict line not found');
  text=text.replace(oldConflicts,newConflicts);
...
// 4) Regression tests for resolved/unresolved warnings and conflicts.
{
  ...
  const newImport="import { normalizeMachineIds, shouldEnforceSelectionQuality, shouldSurfaceResearchWarning, shouldSurfaceResearchConflict, classifyMachineQuality, deriveOverallStatus, generatedPaths } from '../tools/batch-machine-pipeline.mjs';";
