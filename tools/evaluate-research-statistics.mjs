...
function evaluateScalarFeature(f,settingOrder){
  ...
  return {calculable:entries.length>=2,settingCount:entries.length,probabilityRange:ps.length?{min:Math.min(...ps),max:Math.max(...ps)}:null,extremeGap:ps.length?Math.max(...ps)-Math.min(...ps):null,pairwise:pairs,hardestAdjacentPair:hardest};
}
...
export function evaluateResearchData(data){
  ...
  return {evaluatorVersion:"statistical-evaluator-v1.1",researchSchemaVersion:data.schemaVersion,machineId:data.machine?.machineId??null,generatedAt:new Date().toISOString(),disclaimer:"客観的な統計指標のみ。Featureの採用・不採用、重み、実戦評価は決定しない。",features:(data.features??[]).map(f=>evaluateFeature(f,settingOrder))};
}
