...
function ensureReplacement(path, before, after) {
  let text = fs.readFileSync(path, 'utf8');
  if (text.includes(after)) return false;
  const first = text.indexOf(before);
  if (first < 0) throw new Error(`${path}: insertion point not found and replacement is not already applied`);
...
console.log(`Batch identity continuity ensured: ${checkpoint.entries?.length ?? 0} machines`);
