...
// provisionalRegistrationId is the canonical development-time sequence for real machines.
// From 2026-09-12 onward, all currently registered real machines are resequenced by
...
// Explicit test packages are intentionally outside the canonical sequence.
...
if(invalidIntroductionDates.length){
 console.error("Registry resequence blocked: every canonical catalog machine must have a valid introductionDate.");
...
for(const c of catalog.machines??[]){
 ...
 const provisionalRegistrationId=provisionalIdByMachineId.get(c.machineId)??null;
...
console.log(`Canonical provisional registration IDs: 1..${chronologicalOrder.length} by introductionDate (oldest first)`);
const excludedTests=catalogMachines.filter(isTestMachine);
if(excludedTests.length) console.log(`Test packages excluded from canonical IDs: ${excludedTests.map(c=>c.machineId).join(", ")}`);
