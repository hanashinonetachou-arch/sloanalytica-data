...
export function compileEvidenceContract({ selection, research, observation, ui, specification }) {
  ...
  for (const input of projection.inputs) {
    const placements = Object.entries(ui.sections ?? {}).filter(([, section]) => section.inputIds?.includes(input.id)).map(([name]) => name);
    if (placements.length !== 1) blocked(`canonical UI placement is not unique for ${input.id}`);
  ...
  projection.items = projection.items.map(item => {
    ...
    const placements = Object.entries(ui.sections ?? {}).filter(([, section]) => section.inputIds?.includes(item.inputId)).map(([name]) => name);
    if (!spec.canonicalUi || spec.canonicalUi.inputId !== item.inputId || placements.length !== 1 || spec.canonicalUi.section !== placements[0]) blocked(`canonical UI proof missing for ${item.evidenceId}`);
