...
const legacyPackageIds={
  L_MADOKA_FORTE_UU:['INP_EVI_SETTING_FLOOR','INP_EVI_SETTING_DENIAL','INP_EVI_PETIT_CHARACTER_DENIAL'],
  L_TOKYO_GHOUL:['INP_EVI_SETTING_FLOOR','INP_EVI_DENIED_SETTINGS','INP_EVI_SETTING_SET']
};
...
for(const id of ids){
  ...
  if(Object.keys(u.inputContracts??{}).some(k=>k.startsWith('INP_EVI_'))) throw new Error(`${id}: legacy canonical Evidence inputs remain`);
  const raw=fs.readFileSync(`machines/${id}/machine-package.json`,'utf8');
  for(const legacyId of legacyPackageIds[id]) if(raw.includes(legacyId)) throw new Error(`${id}: generated package contains legacy Evidence input ${legacyId}`);
}
