...
export function auditSelectionPolicyMigration(root){
 ...
 for(const de of fs.readdirSync(rr,{withFileTypes:true}).filter(x=>x.isDirectory()&&!x.name.startsWith('_'))){
  ...
  if(generated){
   ...
   if(stable(pubActive)!==stable(genActive)) diffs.push({type:'ACTIVE_FEATURE_SET_DIFF',published:pubActive,generated:genActive});
  ...
  reports.push({machineId:mid,status:error?'BLOCKED':diffs.length?'REVIEW':'PASS',error,diffs});
 }
 return {schemaVersion:'selection-policy-migration-audit-v1',summary:{pass:reports.filter(x=>x.status==='PASS').length,review:reports.filter(x=>x.status==='REVIEW').length,blocked:reports.filter(x=>x.status==='BLOCKED').length},machines:reports};
}
if(import.meta.url===`file://${process.argv[1]}`){const root=path.resolve(process.argv[2]??'.'),r=auditSelectionPolicyMigration(root),out=process.argv[3]??path.join(root,'reports','selection-policy-migration-audit.json');fs.writeFileSync(out,JSON.stringify(r,null,2)+'\n');console.log(`Selection Policy Migration: PASS ${r.summary.pass} / REVIEW ${r.summary.review} / BLOCKED ${r.summary.blocked}`);for(const m of r.machines.filter(x=>x.status!=='PASS'))console.log(m.machineId,m.status,m.error??m.diffs.map(d=>d.type+':'+(d.featureId??'')).join(','));}
