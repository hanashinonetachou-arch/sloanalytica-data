...
for(const id of IDS){
 ...
 for(const f of (sel.features??[]).filter(x=>ADOPTED.has(x.adoptionCategory))){
   for(const inputId of refs(f)) if(!ui.inputContracts?.[inputId]){
     const x=inputMap.get(inputId)??{};
     missing.push({featureId:f.featureId,inputId,name:x.name??null,type:x.type??null,category:x.category??null,inputVisible:x.inputVisible??true,derivedCalculation:x.derivedCalculation??null,derivedFromInputIds:x.derivedFromInputIds??[],description:x.description??null});
   }
 }
 machines.push({machineId:id,status:ui.status,sectionOrder:ui.sectionOrder,missing});
}
const report={schemaVersion:'ui-route-gap-v7-v2',generatedAt:new Date().toISOString(),totalMissing:machines.reduce((n,x)=>n+x.missing.length,0),machines};
