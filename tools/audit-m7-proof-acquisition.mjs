...
export function markdownReport(report) {
  ...
  lines.push("", "## Unresolved blockers", "", "- `L_TENSEI_SHITARA_KEN_DESHITA_GT`: Feature/Evidence sharing formal proof or shared-input contract review is required; no new runtime field is proposed.", "- `LB_FUJIKO_M2`: Selection Quality remains an independent blocker.", "- Label-only candidates require Observation semantics review; all other unproven relationships require explicit lineage annotation.", "- The 21 upstream `FIELD_VERIFICATION_REQUIRED` groups (including the named real-device/service checks) are outside the semantic-review input and remain unchanged.", "", "## Stop statement", "", "No Observation ID was renamed, no meaning was rewritten, and canonical UI, materialized MachineData, App runtime, validator behavior, and real-device status were not changed.");
  return `${lines.join("\n")}\n`;
}
