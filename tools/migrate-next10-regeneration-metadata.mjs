...
function replaceOnceOrAlready(text, from, to, alreadyMarker, label) {
  if (text.includes(alreadyMarker)) return text;
  const first = text.indexOf(from);
  if (first < 0) throw new Error(`${label}: source pattern not found and repair marker absent`);
...
function evidenceKey(e){ return `${e?.inputId ?? ''}\u0000${String(e?.triggerValue ?? '')}`; }
...
for(const id of IDS){
  ...
  const formalEvidences=formal.evidence?.evidences??[];
  const formalEvidenceById=new Map(formalEvidences.map(e=>[e.id,e]));
  const formalEvidenceByContract=new Map();
  for(const e of formalEvidences){
    const key=evidenceKey(e);
    if(formalEvidenceByContract.has(key)) throw new Error(`${id}: duplicate Formal Publish evidence contract ${key}`);
    formalEvidenceByContract.set(key,e);
  }
  for(const e of sel.evidence??[]){
    const src=formalEvidenceById.get(e.evidenceId) ?? formalEvidenceByContract.get(evidenceKey(e));
    if(!src){
      throw new Error(`${id}/${e.evidenceId}: Formal Publish evidence not found by id or input/trigger contract (${e.inputId}/${String(e.triggerValue)})`);
    }
...
builder=replaceOnceOrAlready(builder,
...
builder=replaceOnceOrAlready(builder,
  ...
  'const sourceEvidenceRefs=Array.isArray(e.sourceEvidenceRefs)',
...
validator=replaceOnceOrAlready(validator,
