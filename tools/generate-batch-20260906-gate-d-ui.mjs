...
const semanticLocks={
  L_BURNING_EXPRESS_ZN:['エクスプレスボーナス初当りをactive representativeとし、バーニングボーナスを独立Likelihoodとして復活させない','リールロックFallbackは代表Feature利用時に同時尤度化しない'],
  L_PRISM_NANA_CC:['ボーナス中リールロックの分母はボーナス中ゲーム数。通常Gへ置換しない'],
  L_HIHODEN_PA7:['通常時チェリーは高確率中を除外した通常ゲームだけを分母にする','ボーナス中ハズレはボーナス中ゲームだけを分母にする'],
  ...
  L_TEKKEN_6_YD01H:['ボーナス初当りFallbackはAT初当りと同時尤度化しない','ケロットトロフィーのsource conflictをHard Evidenceへ昇格させない'],
...
function obsForCategory(obs,category){const suffix=String(category).replace(/^SEL_RF_/,'');const direct=(obs.observations??[]).find(o=>o.observationId===`OBS_${suffix}`);return direct?[direct]:[];}
