...
test("controlled lineage queue contains the complete population and no duplicates",()=>{ assert.equal(report.summary.controlledLineageGroupCount,report.groups.length); assert.equal(new Set(keys(report.groups)).size,report.groups.length); });
...
test("topology, subtype, and decision counts close exactly",()=>{ assert.equal(sum(report.summary.topologyCounts),report.summary.controlledLineageGroupCount); assert.equal(sum(report.summary.sourceSetSubtypeCounts),report.summary.controlledLineageGroupCount); assert.equal(sum(report.summary.decisionTypeCounts),report.summary.controlledLineageGroupCount); assert.deepEqual(Object.keys(report.summary.topologyCounts),TOPOLOGIES); assert.deepEqual(Object.keys(report.summary.sourceSetSubtypeCounts),SOURCE_SET_SUBTYPES); assert.deepEqual(Object.keys(report.summary.decisionTypeCounts),DECISION_TYPES); });
test("machine grouping is deterministic and complete",()=>{ assert.equal(report.summary.affectedMachineCount,report.machines.length); assert.deepEqual(report.machines.map(r=>r.machineId),[...report.machines.map(r=>r.machineId)].sort((a,b)=>a.localeCompare(b))); assert.equal(report.machines.reduce((n,r)=>n+r.unresolvedGroupCount,0),report.summary.controlledLineageGroupCount); });
