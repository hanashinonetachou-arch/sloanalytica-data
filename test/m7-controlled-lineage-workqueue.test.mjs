...
test("machine grouping is deterministic and complete",()=>{ assert.equal(report.summary.affectedMachineCount,report.machines.length); assert.deepEqual(report.machines.map(r=>r.machineId),[...report.machines.map(r=>r.machineId)].sort()); assert.equal(report.machines.reduce((n,r)=>n+r.unresolvedGroupCount,0),238); });
