...
test('selection summary readiness audits every current research machine',()=>{
  ...
  const expectedMachineCount=fs.readdirSync(path.join(root,'research'),{withFileTypes:true})
    .filter(d=>d.isDirectory()&&!d.name.startsWith('_'))
    .filter(d=>fs.existsSync(path.join(root,'research',d.name,'research-data.json'))&&fs.existsSync(path.join(root,'research',d.name,'selection-data.json'))).length;
  assert.equal(r.machineCount,expectedMachineCount);
