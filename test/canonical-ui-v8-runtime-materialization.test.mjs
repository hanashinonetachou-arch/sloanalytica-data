...
test("v8 runtime UI is a lossless canonical translation",()=>{const r=materializeCanonicalUiV8(ui);assert.equal(r.source,"CANONICAL_UI");assert.equal(r.accordion.singleOpen,true);assert.equal(r.sections[0].groups[0].inputs[0].label,"通常時ゲーム数");assert.equal(r.sections[1].items[0].interaction.type,"ADD_OBSERVATION");assertCanonicalRuntimeUiEquality(ui,r);});
