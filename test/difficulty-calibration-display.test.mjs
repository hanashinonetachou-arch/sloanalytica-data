...
test('display scores are derived from raw and one global reference',()=>{
 ...
 assert.deepEqual(geass.scores,[{games:1500,score:41},{games:3000,score:63},{games:7000,score:100}]);
});
test('changing global reference recalibrates without changing raw',()=>{
...
test('all scored display scores are non-decreasing with more games',()=>{
 const d=JSON.parse(fs.readFileSync('difficulty-catalog.json'));
 for(const e of d.entries){if(e.difficulty?.status!=='SCORED')continue; const a=e.difficulty.scores; assert.ok(a[0].score<=a[1].score&&a[1].score<=a[2].score,e.machineId);}
});
