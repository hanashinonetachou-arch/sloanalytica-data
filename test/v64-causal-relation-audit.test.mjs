...
test('all current causal-review candidates have explicit semantic classification',()=>{
  ...
  assert.equal(report.summary.candidateCount,18);
  assert.equal(report.summary.machineCount,7);
  assert.deepEqual(report.summary.relationCounts,{
    CONDITIONAL_COMPOSITION:5,
    CAUSALLY_RELATED_BUT_DISTINCT_OBSERVATION:10,
